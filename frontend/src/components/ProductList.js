...
import { Container, Alert, Button, Pagination, Form, Row, Col, Card, ListGroup, ButtonGroup } from 'react-bootstrap';
...
const ProductCard = ({ user, product, onAdjustClick, onEditClick, onDeleteClick }) => {
    ...
    return (
        <Card className="h-100 card-lift">
            <Link to={`/products/${product.id}`}><Card.Img variant="top" src={product.images && product.images.length > 0 ? product.images[0].image : 'https://via.placeholder.com/300x200.png?text=No+Image'} className="product-card-img" /></Link>
...
const ProductList = () => {
        ...
        api.get(apiUrl)
            .then(response => {
                ...
                setPagination({
                    ...
                    currentPage: Number(currentPageParams.get('page')) || 1
                });
            })
    ...
    React.useEffect(() => {
        ...
        if (!token) return;
        const socket = new WebSocket(`wss://inventory-pro-49re.onrender.com/ws/products/?token=${token}`);
                            ...
                            <Form onSubmit={(e) => { e.preventDefault(); handleApplyFilters(); }}>
                                <Row className="align-items-end">
                                    ...
                                    <Col md={3} className="d-grid gap-2 d-md-flex justify-content-md-end align-items-end">
                                        <Button variant="outline-secondary" type="button" onClick={handleClearFilters}>Clear Filters</Button>
